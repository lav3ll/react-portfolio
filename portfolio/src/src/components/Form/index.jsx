...
function Form() {
  ...
  const handleInputChange = (event) => {
    ...
    // When first name is updated setFormData({firstName:value. lastName: formData.value})

    // When last name is updated setFormData({firstName:formData.firstName lastName: value})
    setFormData({
      ...formData,
      [name]: value,
    });
  ...
  const handleFormSubmit = (event) => {
    ...
    if (formData.firstName === "" || formData.lastName === "") {
      alert(`You didn't fill out all the inputs`);
      return;
    }
